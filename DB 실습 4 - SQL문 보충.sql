...
전화번호 char(16),
위치 varchar(30),
primary key (학과번호)) engine = innodb default charset=utf8;
...
담당교수 char(10),
강의실 varchar(30),
primary key(과목번호)) engine = innodb default charset=utf8;
...
foreign key(과목번호) references 과목(과목번호)) engine = innodb default charset=utf8;
...
select 학번, 이름, 학과명 from 학생 as s, 학과 as d
...
select 학번, 이름 from 학생 as s where exists
(select * from 학과 as d where s.학과번호 = d.학과번호 and 학과명='물리학과');
